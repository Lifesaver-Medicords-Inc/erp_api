...
func GetItemRequest(c *fiber.Ctx) error {
	data, status, err := setup_services.GetItemRequest(nil)
	if err != nil {
...
func CreateItemRequest(c *fiber.Ctx) error {
	...
	data, status, err := setup_services.CreateItemRequest(c, tx)
	if err != nil {
		tx.Rollback()
		return utils.RespondError(c, status, err.Error())
	}
...
func UpdateItemRequest(c *fiber.Ctx) error {
	...
	data, status, err := setup_services.UpdateItemRequest(c, tx, nil)
	if err != nil {
		tx.Rollback()
		return utils.RespondError(c, status, err.Error())
	}
...
func DeleteItemRequest(c *fiber.Ctx) error {
	...
	data, status, err := setup_services.DeleteItemRequest(c, tx, nil)
	if err != nil {
		tx.Rollback()
		return utils.RespondError(c, status, err.Error())
	}
