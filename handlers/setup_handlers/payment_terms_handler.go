...
func GetPaymentTerm(c *fiber.Ctx) error {
	...
	data, status, err := setup_services.GetPaymentTerm(idNum)
	if err != nil {
		return utils.RespondError(c, status, err.Error())
	}
...
func CreatePaymentTerms(c *fiber.Ctx) error {
	...
	data, status, err := setup_services.CreatePaymentTerms(c, tx)
	if err != nil {
		tx.Rollback()
		return utils.RespondError(c, status, err.Error())
	}
...
func UpdatePaymentTerms(c *fiber.Ctx) error {
	...
	data, status, err := setup_services.UpdatePaymentTerms(c, tx, nil)
	if err != nil {
		tx.Rollback()
		return utils.RespondError(c, status, err.Error())
	}
...
func DeletePaymentTerms(c *fiber.Ctx) error {
	...
	data, status, err := setup_services.DeletePaymentTerms(c, tx, nil)
	if err != nil {
		tx.Rollback()
		return utils.RespondError(c, status, err.Error())
	}
