CREATE   VIEW [dbo].[vw_get_engineering_redbox_quotation_list] AS
SELECT
    sq.id AS id,
...
INNER JOIN tbl_bpi bpi
    ON sq.customer_id = bpi.id
INNER JOIN tbl_trans_sales_opportunity so 
    ON sq.document_no = so.document_no
