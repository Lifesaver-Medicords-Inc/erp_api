CREATE VIEW [dbo].[vw_get_purchasing_so_purchase_list] AS
...
SELECT
    ...
    STUFF((
        ...
        ORDER BY a2.order_details_id
        FOR XML PATH(''),TYPE).value('.','NVARCHAR(MAX)'),1,1,'') AS order_ids,
    ...
    STUFF((
        ...
        AND ISNULL(a2.qty,0) - ISNULL(a2.allocated_qty,0) > 0
        ORDER BY a2.order_details_id
        FOR XML PATH(''),TYPE).value('.','NVARCHAR(MAX)'),1,1,'') AS order_detail_ids,
    ...
    STUFF((
        ...
        AND ISNULL(a2.qty,0) - ISNULL(a2.allocated_qty,0) > 0
        ORDER BY a2.order_details_id
        FOR XML PATH(''),TYPE).value('.','NVARCHAR(MAX)'),1,1,'') AS sales_order_nos,
    ...
    STUFF((
        ...
        AND ISNULL(a2.qty,0) - ISNULL(a2.allocated_qty,0) > 0
        ORDER BY a2.order_details_id
        FOR XML PATH(''),TYPE).value('.','NVARCHAR(MAX)'),1,1,'') AS project_names,
    ...
    STUFF((
        ...
        AND ISNULL(a2.qty,0) - ISNULL(a2.allocated_qty,0) > 0
        ORDER BY a2.order_details_id
        FOR XML PATH(''),TYPE).value('.','NVARCHAR(MAX)'),1,1,'') AS sales_executives,
    ...
    STUFF((
        ...
        AND ISNULL(a2.qty,0) - ISNULL(a2.allocated_qty,0) > 0
        ORDER BY a2.order_details_id
        FOR XML PATH(''),TYPE).value('.','NVARCHAR(MAX)'),1,1,'') AS qtys,
    ...
    STUFF((
        ...
        AND ISNULL(a2.qty,0) - ISNULL(a2.allocated_qty,0) > 0
        ORDER BY a2.order_details_id
        FOR XML PATH(''),TYPE).value('.','NVARCHAR(MAX)'),1,1,'') AS unit_prices,
    ...
    STUFF((
        ...
        AND ISNULL(a2.qty,0) - ISNULL(a2.allocated_qty,0) > 0
        ORDER BY a2.order_details_id
        FOR XML PATH(''),TYPE).value('.','NVARCHAR(MAX)'),1,1,'') AS discounts,
    ...
    STUFF((
        ...
        AND ISNULL(a2.qty,0) - ISNULL(a2.allocated_qty,0) > 0
        ORDER BY a2.order_details_id
        FOR XML PATH(''),TYPE).value('.','NVARCHAR(MAX)'),1,1,'') AS commitment_dates,
