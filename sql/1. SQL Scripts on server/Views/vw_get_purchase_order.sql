CREATE VIEW [dbo].[vw_get_purchase_order] AS
WITH pod_with_received AS (
    SELECT
        pod.based_id AS purchase_order_id,
        pod.id AS pod_id,
        ...
        CAST(pod.order_qty AS INT)
            - ISNULL(SUM(CAST(wh.received_qty AS INT)), 0) AS balance_qty
    FROM tbl_purchasing_purchase_order_details pod
    LEFT JOIN tbl_inv_warehouse_receiving_history wh
