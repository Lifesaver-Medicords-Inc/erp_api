CREATE VIEW [dbo].[GetBpiGeneralList] AS
SELECT  
 a.id AS general_id,  
...
LEFT JOIN (
    SELECT 
        ...
        STUFF((
            ...
            WHERE t2.bpi_general_id = t1.bpi_general_id
            FOR XML PATH(''), TYPE
        ...
        STUFF((
            SELECT ',' + bb.code
            FROM tbl_bpi_branch_industries t2
            LEFT JOIN tbl_setup_bpi_industries bb 
                ON t2.industry_id = bb.id
            WHERE t2.bpi_general_id = t1.bpi_general_id
            FOR XML PATH(''), TYPE
...
LEFT JOIN (
    SELECT 
        ...
        STUFF((
            SELECT ',' + bb.code
            FROM tbl_bpi_entity t2
            LEFT JOIN tbl_setup_bpi_entity bb 
                ON t2.entity_id = bb.id
            WHERE t2.bpi_general_id = t1.bpi_general_id
            FOR XML PATH(''), TYPE
