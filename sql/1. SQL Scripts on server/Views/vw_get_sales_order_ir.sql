CREATE VIEW [dbo].[vw_get_sales_order_ir] AS
WITH sod_with_received AS (
    SELECT
        ...
        CAST(sod.qty AS INT)
            - ISNULL(SUM(CAST(ih.req_qty AS INT)), 0) AS balance_qty
    FROM tbl_trans_sales_order_details sod
    LEFT JOIN tbl_inv_item_request_history ih
        ON sod.order_details_id = ih.sod_id
    GROUP BY
        sod.based_id,
