CREATE PROCEDURE [dbo].[sp_GetJobOrders]
    @UserId INT
AS
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN TRY
        SELECT
			ISNULL(jo.job_order_id, 0) AS id,
		...
		INNER JOIN tbl_setup_item_additional_specs ias
			ON ias.based_id = i.id
        LEFT JOIN dbo.tbl_trans_job_order AS jo
            ON sod.order_details_id = jo.order_details_id
