CREATE PROCEDURE [dbo].[sp_GetBpiList]
AS
BEGIN
  SELECT
        a.id AS id,
    ...
    LEFT JOIN (
		SELECT
				ISNULL(STRING_AGG(b.industry_id, ','),'')  AS industry_ids,
			...
			LEFT JOIN (
				...
				GROUP BY bb.id,aa.bpi_id,bb.code,aa.industry_id
			) b ON a.bpi_id = b.bpi_id AND a.industry_id = b.id
			GROUP by b.bpi_id
	) c on a.id = c.bpi_id
