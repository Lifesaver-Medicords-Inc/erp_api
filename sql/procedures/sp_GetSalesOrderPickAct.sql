IF NOT EXISTS (SELECT 1 FROM sys.procedures WHERE name = 'sp_GetSalesOrderPickAct' AND schema_id = SCHEMA_ID('dbo'))
BEGIN
    EXEC('CREATE PROCEDURE [dbo].[sp_GetSalesOrderPickAct] AS SET NOCOUNT ON;')
END
GO
