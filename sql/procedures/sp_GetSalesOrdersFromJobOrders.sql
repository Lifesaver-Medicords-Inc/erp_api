IF NOT EXISTS (SELECT 1 FROM sys.procedures WHERE name = 'sp_GetSalesOrdersFromJobOrders' AND schema_id = SCHEMA_ID('dbo'))
BEGIN
    EXEC('CREATE PROCEDURE [dbo].[sp_GetSalesOrdersFromJobOrders] AS SET NOCOUNT ON;')
END
GO
ALTER PROCEDURE [dbo].[sp_GetSalesOrdersFromJobOrders] @SalesOrder VARCHAR(50) AS BEGIN
SET NOCOUNT ON;
BEGIN TRY
