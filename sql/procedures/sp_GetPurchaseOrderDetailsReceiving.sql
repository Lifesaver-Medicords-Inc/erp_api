IF NOT EXISTS (SELECT 1 FROM sys.procedures WHERE name = 'sp_GetPurchaseOrderDetailsReceiving' AND schema_id = SCHEMA_ID('dbo'))
BEGIN
    EXEC('CREATE PROCEDURE [dbo].[sp_GetPurchaseOrderDetailsReceiving] AS SET NOCOUNT ON;')
END
GO
ALTER PROCEDURE [dbo].[sp_GetPurchaseOrderDetailsReceiving] @PurchaseId INT AS BEGIN
SET NOCOUNT ON;
BEGIN TRY
