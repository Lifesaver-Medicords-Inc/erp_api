IF NOT EXISTS (SELECT 1 FROM sys.procedures WHERE name = 'sp_GetSalesOrderInvoice' AND schema_id = SCHEMA_ID('dbo'))
BEGIN
    EXEC('CREATE PROCEDURE [dbo].[sp_GetSalesOrderInvoice] AS SET NOCOUNT ON;')
END
GO
