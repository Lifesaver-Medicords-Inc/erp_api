IF NOT EXISTS (SELECT 1 FROM sys.views WHERE name = 'vw_get_purchasing_canvass_sheet_so' AND schema_id = SCHEMA_ID('dbo'))
BEGIN
    EXEC('CREATE VIEW [dbo].[vw_get_purchasing_canvass_sheet_so] AS SELECT 1 AS placeholder')
END
GO
...
SELECT id,
    ...
    CASE
        WHEN DATEDIFF(
            ...
            DATEADD(DAY, price_validity, CAST(start_date AS DATE))
        ) >= 0 THEN DATEDIFF(
            DAY,
            CAST(GETDATE() AS DATE),
            DATEADD(DAY, price_validity, CAST(start_date AS DATE))
