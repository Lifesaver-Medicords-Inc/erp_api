IF NOT EXISTS (SELECT 1 FROM sys.views WHERE name = 'vw_get_purchasing_redbox_sales_order_list' AND schema_id = SCHEMA_ID('dbo'))
BEGIN
    EXEC('CREATE VIEW [dbo].[vw_get_purchasing_redbox_sales_order_list] AS SELECT 1 AS placeholder')
END
GO
ALTER VIEW [dbo].[vw_get_purchasing_redbox_sales_order_list] AS
    ...
    ISNULL(
        STUFF(
            (
                ...
                ORDER BY a.order_id ASC FOR XML PATH('')
            ),
            1,
            1,
            ''
