IF NOT EXISTS (SELECT 1 FROM sys.views WHERE name = 'vw_get_purchasing_list_supplier' AND schema_id = SCHEMA_ID('dbo'))
BEGIN
    EXEC('CREATE VIEW [dbo].[vw_get_purchasing_list_supplier] AS SELECT 1 AS placeholder')
END
GO
...
FROM tbl_bpi a
    ...
    LEFT JOIN tbl_bpi_address i ON b.id = i.branch_id -- OUTER APPLY for distinct addresses
    OUTER APPLY (
        SELECT STUFF(
                (
                    ...
                    WHERE a2.branch_id = b.id
                        AND a2.location IS NOT NULL FOR XML PATH(''),
                        TYPE
                ).value('.', 'NVARCHAR(MAX)'),
                1,
                2,
    ...
    ) addresses -- OUTER APPLY for item IDs
    OUTER APPLY (
        SELECT STUFF(
                (
                    SELECT DISTINCT ', ' + CAST(h2.item_id AS NVARCHAR(MAX))
                    FROM tbl_bpi_items h2
                    WHERE h2.branch_id = b.id
                        AND h2.item_id IS NOT NULL FOR XML PATH(''),
                        TYPE
                ).value('.', 'NVARCHAR(MAX)'),
                1,
                2,
